...
def select_small_cubic_1(data_size, data_indices, whole_data, patch_length, padded_data, dimension):
    small_cubic_data = np.zeros((data_size, 2 * patch_length + 1, 2 * patch_length + 1, dimension)) #patch大小的数据立方体
    data_assign = index_assignment(data_indices, whole_data.shape[0], whole_data.shape[1], patch_length) #把列表中的索引转变为2维坐标
    for i in range(len(data_assign)): #把所有的Patch选出来
...
#背景变为-1
    ...
    x1_tensor_train = torch.from_numpy(X1_train).type(torch.FloatTensor).unsqueeze(1)#通过torch.from_numpy创建张量；通过unsuqeeze(int)中的int整数，增加一个维度，int整数表示维度增加到哪儿去，且维度为1。
    ...
    torch_dataset_total = Data.TensorDataset(X1_total_tensor_data, X2_total_tensor_data)
