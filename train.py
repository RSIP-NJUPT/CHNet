...
def loadData():
    # 读入数据
    data_HSI = opt.data_HSI
    data_lidar = opt.data_lidar
    labels = opt.labels
...
def create_data_loader():
    ...
    ALL_SIZE = opt.ALL_SIZE
    
    # 使用 PCA 降维，得到主成分的数量
    pca_components = opt.pca_components
...
if __name__ == '__main__':
    for i in range(opt.n):
        ...
        net, device = train(train_iter, opt.epoches)
        # 只保存模型参数
        toc1 = time.perf_counter()
        torch.save(net.state_dict(), opt.note_1+"_{}.pth".format(i))
        ...
        Test_time = toc2 - tic2
        file_name = opt.note+"_{}.txt".format(i)
